...
if __name__ == '__main__':
    ...
    from data_utils import imshow
    import numpy as np
    import matplotlib.pyplot as plt
    train_transform = prepare_transform('cifar10')
    train_dataset = prepare_datasets('cifar10', train_transform)
    train_dataloader = prepare_dataloader(train_dataset, batch_size=4, num_workers=0)

    # get some random training images
    # dataiter = iter(train_dataloader)
    # imagesq, target= dataiter.next()
    ...
    plt.subplot(3,1,1)
    plt.imshow(tensor_to_array(img1))
    plt.subplot(3,1,2)
    plt.imshow(tensor_to_array(img2))
    plt.subplot(3,1,3)
    plt.imshow(tensor_to_array(img2))
    plt.show()
    print(label)
